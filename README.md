...
| How it works | the four moves in order (host a folder, send the invite, type in the same file, close the window), under its own one-line summary of the workflow. They are the sequence rather than the argument: the grant, the sealed material and the host's signed claim are all made in the sections above, where a reader meets the thing they are about. The fourth move says whose window closes and what a dropped connection does not do: the host's window ends the room after a short countdown, a guest's closing leaves the others in it, and a connection that only drops can rejoin inside the countdown |
...
- **A number on the page has a home.** The counts the page shows (the wire layer's 24 vectors, 33,760
  frame checks and 8,387 assertions; the peer layer's 26 vectors, 221 checks and 74 assertions are the
